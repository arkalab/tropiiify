...
class TropiiifyPlugin {
  ...
  async export(data) {
    ...
    const map = this.mapLabelsToIds(
      this.loadTemplate(this.options.itemTemplate)
    );
    ...
    if (missingIds) {
      this.context.dialog.notify("missing.ids", {
        message:
          "Every item must have an identifier. Please review your project and try again.",
        type: "info",
      });
      return;
    } else {
  ...
  createCollection(items) {
    ...
    return collectionBuilder.toPresentation3({
      id: collection.id,
      type: "Collection",
    });
  }
    ...
    if (!template || template.id === "https://tropy.org/v1/templates/generic") {
      ...
      for (let { label, property } of template.fields) {
        label
          .split("|")
          .map((label) =>
            label.replaceAll(/:(\w)/g, (_, char) => char.toUpperCase())
          )
  ...
  async handleImages(item) {
    const promises = item.photo.map(async (photo) => {
      ...
      const thumbPromise = this.processImage(
        sharpInstance.clone(),
        300,
        item,
        photo
      ...
      const midsizePromise = this.processImage(
        sharpInstance.clone(),
        Math.min(2000, Math.max(photo.width, photo.height)),
        item,
        photo
      );

      // Wait for both thumbnail and midsize promises to resolve
      const [thumbSize, midSize] = await Promise.all([
        thumbPromise,
        midsizePromise,
      ]);
      ...
      await sharpInstance
        .clone()
        .tile({ layout: "iiif3", id: item.baseId })
        .toFile(tilesPath);
