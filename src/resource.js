...
class Resource {
  ...
  constructor(data = {}, map, options = {}) {
    ...
    this.photo = this.extractValue(data, tropy("photo")).map((photo) => ({
      ...
      rotation: this.extractValue(photo, tropy("angle")),
      mirror: this.extractValue(photo, tropy("mirror")),
      mimetype: this.extractValue(photo, tropy("mimetype")),
      path: this.extractValue(photo, tropy("path")),
      note: this.extractValue(
        this.extractValue(photo, tropy("note"))?.[0],
        tropy("html")
      ),
      selection: this.extractValue(photo, tropy("selection"))?.map(
        (selection) => ({
          note: this.extractValue(
            this.extractValue(selection, tropy("note"))?.[0],
            tropy("html")
          ),
  ...
  extractValue(data, property) {
    return (
      data?.[property]?.[0]?.["@value"] || data?.[property]?.[0]?.["@list"]
    );
  }
  ...
  createManifest(sizes) {
    const normalizedManifest = manifestBuilder.createManifest(
      ...
      (manifest) => {
        ...
        this.requiredstatementValue &&
          manifest.setRequiredStatement({
            label: { none: [this.options.requiredStatementLabel] },
            value: {
              none: [
                `${this.options.requiredStatementText} ${this.assembleHTML(
    ...
    return manifestBuilder.toPresentation3({
